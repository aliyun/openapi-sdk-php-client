...
namespace AlibabaCloud\Client\Tests\Functional\Credentials;
 ...
 *
 * @package      AlibabaCloud\Client\Tests\Functional\Credentials
 *
 * @author       Alibaba Cloud SDK <sdk-team@alibabacloud.com>
...
class RamRoleArnCredentialTest extends TestCase
{
    ...
    {
        try {
            $request = (new DescribeRulesRequest())->client($this->clientName)
    ...
    {
        try {
            $result = (new ListAccessKeysRequest())->client($this->clientName)
                                                   ->setUserName(\time())
                                                   ->request();
        } catch (ClientException $e) {
            self::assertEquals(\ALI_SERVER_UNREACHABLE, $e->getErrorCode());
